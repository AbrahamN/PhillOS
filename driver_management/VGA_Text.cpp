...
ShortPositiveType CursorPos = 0; 		// Holds the current position of the cursor


void SetCursorPosRaw(ShortPositiveType pos){	// Does some I/O black magic 
	if(pos >= 0 && pos < 2000) {
		outb(0x3d4, 0x0f);
		outb(0x3d5, (ByteType)(pos & 0xff));
		outb(0x3d4, 0x0e);
		outb(0x3d5, (ByteType)((pos >> 8) & 0xff));
		CursorPos = pos;
	}
	return;
}
...
/*ShortPositiveType GetCursorPos()		//Commented for no particular reason, uncomment if needed
{
	ShortPositiveType pos = 0;
	outb(0x3D4, 0x0F);
	pos |= inb(0x3D5);
	outb(0x3D4, 0x0E);
	pos |= ((ShortPositiveType)inb(0x3D5)) << 8;
	return pos;
}
...
void kprint(const char* s){		// Just a simple print function. Prints to screen at cursor position, moves the cursor at the end. 
	ByteType* charPtr = (ByteType*)s;
	ShortPositiveType i = CursorPos;
	while(*charPtr != 0){
	switch (*charPtr) {
		case 10:	if(i < 1920){
                        if(CursorPos >= 1760){
